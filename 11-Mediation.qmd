...
plotdf <- data.frame(effecttype = c("Effect of Democracy \n without Fixing Threat (ATE)", 
                        "Effect of Democracy \n Fixing Threat (ACDE)", 
                        "Eliminated Effect = ATE - ACDE"),
           estimate= c(ate, acde, aee),
           ymin = c((ate - 1.96*se.sum), confint(fit)[2,1], confint(fit)[4,1]
                    ),
           ymax = c((ate + 1.96*se.sum),confint(fit)[2,2], 
                    confint(fit)[4,2]))

library(ggplot2)
ggplot(plotdf, aes(x=effecttype, y=estimate,
                   ymin=ymin, ymax=ymax))+
  geom_point()+
  ...
  coord_flip()+
  theme_bw()+
  labs(title="Effect on Supporting Preemptive Strike",
       y="Tomz and Weeks Extension", x="")
```
