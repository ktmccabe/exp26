...
<summary>Below, note its equivalence to regression.</summary>
...
However, let's move to an uncertainty calculation that may be more appropriate for dichotomous variables:
...
<summary>Below, note the uncertainty calculation for a difference in proportions.</summary>
...
Another alternative you can consider with a dichotomous outcome would be to run a logistic regression. However, some may recommend this, as the coefficients cannot be directly interpreted as the average treatment effects. Robin Gomila has a recent paper discussing the tradeoffs in using linear regression (preferred by the paper) vs. logistic regression in these situations. See paper [here](https://www.robingomila.com/files/publications_pdfs/Gomila_2020_Logistic_vs_Linear.pdf). When using linear regression with a dichotomous outcome, we call this a "linear probability model." In these cases, you should probably use heteroskedasticity-consistent standard errors due to the structure of what errors will look like in the model when an outcome only takes the values 0 and 1. The `lm_robust` regression function from the `library(estimatr)` R package uses these by default.
...
<summary>Below, we can calculate this by hand.</summary>
...
We also might have designs that combine the pre-post framework with a between-subjects component. Suppose I have a between-subjects and within-subjects survey experiment where I measure feeling thermometer pre-treatment, have an experimental intervention where subjects are assigned to treatment or control, and then re-measure the feeling thermometer. We might ask how can I analyze this effectively using a regression model that takes into account the repeated measures.

There are multiple approaches to this, but a common way to do this is to 1) stack the data in "long" format. If you have 500 subjects, imagine a 1000-observation dataset with a column for the FT outcome, a column for treatment status, and a column for whether the FT measure was pre- or post-treatment. We can then use an interaction model where we interact a subject's treatment assignment with an indicator for which wave of the data we are measuring. The interaction term from the model will reflect our estimate of the average causal effect because it suggests how much the change in FT depended on the treatment assignment.

Here is imagined code for this using the random effects framework, which helps to model unit dependence, given that a subject is represented twice in the data:
...
datalong <- df %>%
  pivot_longer(
    cols = c(thermo_pre, thermo_post),
    names_to = "time",
    values_to = "thermo"
  ) %>%
  mutate(
    post = if_else(time == "thermo_post", 1, 0),
    treat = as.integer(treat)
  )

# Repeated-measures model
fit1 <- lmer(thermo ~ post * treat + (1 | id), data = datalong)
summary(fit1)
```

For a recent paper that discusses upsides and downsides of repeated measures designs, see: JORDAN D, OLLERENSHAW T, TREXLER A. New Evidence and Design Considerations for Repeated Measure Experiments in Survey Research. American Political Science Review. Published online 2026:1-21. doi:10.1017/S0003055426101671

## Using covariates

Two additional topics to consider would be how best to integrate covariates into experimental analysis.

One approach is to use covariates to help increase the precision of your experimental effects estimates. See Andrew Gelman's take on this here: <https://statmodeling.stat.columbia.edu/2024/11/18/5-different-reasons-why-its-important-to-include-a-pre-treatment-variable-when-designing-and-analyzing-a-randomized-experiment/>.

An estimator one can use for this is from Winston Lin's paper on regression adjustment. He describes this work here: <https://blogs.worldbank.org/en/impactevaluations/regression-adjustment-in-randomized-experiments-is-the-cure-really-worse-than-the-disease>. The estimatr package implements this fairly easily with the `lm_lin` function: <https://declaredesign.org/r/estimatr/reference/lm_lin.html>.

Another is to use pre-treatment covariates to help identify heterogeneous treatment effects. Egami, Imai, and Ratkovic have a paper and R package that provides an idea of how to pursue this: <https://search.r-project.org/CRAN/refmans/FindIt/html/FindIt-package.html>
