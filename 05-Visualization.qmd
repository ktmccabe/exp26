...
Here is a short video from a previous version of the course, where this session was asynchronous, walking through the code to plot the ATEs using `plot` and `ggplot`. (Via youtube, you can speed up the playback to 1.5 or 2x speed.)
...
outp <- avg_comparisons(
  fit,
  variables = list(angvcon = c(0, 1)),
  by = "blackauto3"
)
outp
```
...
outp2 <- avg_predictions(fit, variables = c("blackauto3", "angvcon"),
                         newdata=datagrid(blackauto3 = c(0, 1, 2),
                                  angvcon = c(0,1), grid_type = "counterfactual"))
...
We could instead create a barplot.
...
Also note that if you don't wish to show the uncertainty, you can get a barplot more easily by doing this:

```{r}
agg <- aggregate(blackdon ~ angvcon + blackauto3, data=angcontrol, mean)
agg

ggplot(agg, aes(x=blackauto3, y=blackdon,
                     fill=as.factor(angvcon)))+
  geom_col(position=position_dodge(.9))+
  theme_bw()+
  ggtitle("Average Donations to Black Organizations \n by Community nationalism")+
